#!/usr/bin/env bash
# Build zh_tts normalizer into e2e_test/bin/
set -euo pipefail
...
OUT_BIN="${BIN_DIR}/zh_tts"

if [[ ! -f "${TN_ROOT}/zh.cpp" ]]; then
...
echo "[build] Compiling zh_tts (ICU_ROOT=${ICU_ROOT})..."
g++ -std=c++17 -O2 \
  "${TN_ROOT}/zh.cpp" "${TN_ROOT}/tts_normalizer_engine.cpp" "${TN_ROOT}/ru_year_spellout.cpp" \
